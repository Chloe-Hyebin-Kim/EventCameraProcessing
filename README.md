...
**Camera Bias** 그룹박스: Live camera가 실제로 열려 있을 때만(`Start` 성공 후) 채워짐 - Metavision HAL의 `I_LL_Biases` 파실리티(`Camera::get_facility<I_LL_Biases>()`)에서 `get_all_biases()`로 IMX636의 현재 bias 목록(`bias_diff`, `bias_diff_off`, `bias_diff_on`, `bias_fo`, `bias_hpf`, `bias_refr` 등, 카메라/펌웨어에 따라 다를 수 있음)을 읽어와 bias마다 슬라이더 한 줄씩 동적으로 생성함. 슬라이더 범위는 `LL_Bias_Info::get_bias_range()`(권장 범위, `DeviceConfig::biases_range_check_bypass`를 켜지 않는 한), 슬라이더를 움직이면 즉시 `I_LL_Biases::set()`으로 카메라에 반영됨. `LL_Bias_Info::is_modifiable()`이 false인 bias는 슬라이더가 비활성화된 채로 현재 값만 표시함. RAW 파일 재생이나(파실리티 자체가 없음) Stop 상태에서는 "Not available" 안내만 표시되고 슬라이더는 모두 비워짐. Bias 이름/설명 문구는 하드웨어가 보고하는 그대로(로그와 마찬가지로 언어 설정과 무관하게 항상 영어)임.
